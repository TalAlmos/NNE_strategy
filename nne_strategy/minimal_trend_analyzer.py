...
def fetch_and_save_data(ticker, date):
    """
    Fetches data for the given ticker and date.
    ...
    Returns:
        pandas.DataFrame: DataFrame with OHLC price data
        str: File path of the data file
    """
    # Construct the file path based on the ticker and date
    file_path = Path(f"nne_strategy/data/raw/{ticker}_data_{date.replace('-', '')}.csv")
...
def analyze_trends(df):
    ...
    SWING_THRESHOLD = 0.50  # Minimum price movement to confirm trend change ($)
    CONFIRMATION_PERIODS = 3  # Number of periods to confirm trend change
    VOLUME_FACTOR = 1.5  # Volume increase factor to help confirm trend change
    MIN_TREND_DURATION = 5  # Minimum number of periods for a valid trend
...
def save_trend_analysis(df, date, ticker):
    ...
    with open(summary_file, 'w') as f:
        ...
        for trend_num, group in df.groupby(trend_changes):
            ...
            start_time = pd.to_datetime(group['Datetime'].iloc[0])
            end_time = pd.to_datetime(group['Datetime'].iloc[-1])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Please use format YYYYMMDD (e.g., 20241205)")
            sys.exit(1)
    else:
        DATE = "2024-12-05"  # Default date
